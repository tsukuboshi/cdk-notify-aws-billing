...
export class NotifyAwsBillingStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const notifyDaysInterval = this.node.tryGetContext('notifyDaysInterval') as string;
    const slackWebhookUrlPath = this.node.tryGetContext('slackWebhookUrlPath') as string | "";
    const lineAccessTokenPath = this.node.tryGetContext('lineAccessTokenPath') as string | "";
    ...
    const notifyAwsBillingRole = new iam.Role(this, 'notifyAwsBillingRole', {
      ...
      inlinePolicies: {
        'NotifySlackToBilling': new iam.PolicyDocument({
          statements: [
    ...
    // Lambda function
    const notifyAwsBillingFunction = new lambdaPython.PythonFunction(this, 'function', {
    ...
    new events.Rule(this, 'notifyAwsBillingRule', {
      ...
      schedule: events.Schedule.cron({ day: `*/${notifyDaysInterval}`, hour: '0', minute: '0' }),
